...
var ResultTable = Class.create({
  ...
  initialize: function(element,rElement,url,view,tab,index,doc,page,zoom,rotate,download,initmode) {
    ...
    this.timer = '';
    this.initialized = 0;
    // this.bs = 20; // firefox 4/5/6.. stopped after 100 documents
    this.bs = 21;
    this.side = '';
  ...
  go_action_f4: function(key,shft,alt,ctrl) {
    if (shft==0 && ctrl==0) {
      ...
      if (shft==1 && ctrl==0) {
		...
		if (shft==1 && ctrl==1) {
		  // ctrl+f4 is used in firefox to close tab, so use shft+ctrl+f4
      GRT.docs_next();
		}
    return false;  
  },
  ...
  checkConfirm: function(mainview,mess1,mess2,mess3,mess4){
    var nrOfDocsSel = 0;
